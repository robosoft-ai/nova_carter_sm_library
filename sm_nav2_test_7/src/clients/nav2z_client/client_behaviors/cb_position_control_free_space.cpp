...
void CbPositionControlFreeSpace::onEntry()
{
  ...
  while (rclcpp::ok() && !goalReached_)
  {
    ...
    double distance_to_target = std::sqrt(error_x * error_x );

    double alpha = atan2(error_y, error_x);
    ...
    else
    {
      ...
      RCLCPP_INFO_STREAM(
        getLogger(), "[" << getName() << "] cmd_linear_x: " << cmd_linear_x << ", cmd_angular_z: " << cmd_angular_z
      );
