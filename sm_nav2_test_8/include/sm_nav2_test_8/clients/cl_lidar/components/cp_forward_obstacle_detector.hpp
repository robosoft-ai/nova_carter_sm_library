...
public:
  ...
  float selected_forward_distance = std::numeric_limits<float>::quiet_NaN();
  ...
  {
    return selected_forward_distance;
  }
  ...
  void MessageCallbackStoreDistanceToWall(const sensor_msgs::msg::LaserScan & scanmsg)
  {
    ...
    {
      RCLCPP_INFO_STREAM_THROTTLE(getLogger(), *(getNode()->get_clock()), 500, "[" << getName() << "] fwdist: " << fwdist);
      selected_forward_distance = std::max(fwdist - SECURITY_DISTANCE, 0.0F);
    }
